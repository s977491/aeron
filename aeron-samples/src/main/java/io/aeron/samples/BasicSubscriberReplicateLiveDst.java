 ...
 */
public class BasicSubscriberReplicateLiveDst {
    ...
    public static void main(final String[] args) {
        ...
        final Aeron.Context ctx = new Aeron.Context()
                ...
                .unavailableImageHandler(BasicSubscriberReplicateLiveDst::unavail);

        if (EMBEDDED_MEDIA_DRIVER) {
            ctx.aeronDirectoryName(driver.aeronDirectoryName());
        }

        final FragmentHandler fragmentHandler = BasicSubscriberReplicateLiveDst::handler;
        ...
        try (Aeron aeron = Aeron.connect(ctx);
             Subscription subscription = aeron.addSubscription("aeron:udp?endpoint=239.255.255.255:3333", STREAM_ID)) {
    ...
    private static void handler(DirectBuffer buffer, int offset, int length, Header header) {
        final byte[] data = new byte[length];
        buffer.getBytes(offset, data);

        System.out.println(String.format(
                "Message to stream %d from session %d (%d@%d) <<%s>>",
                header.position(), header.sessionId(), length, offset, new String(data)));
    }
}
